...
namespace Quasar.Client.Networking
{
    public class Client : ISender
    {
        ...
        /// 表示处理客户端状态变化的方法。
        /// </summary>
        /// <param name="s">状态发生变化的客户端。</param>
        /// <param name="connected">客户端的新连接状态。</param>
        public delegate void ClientStateEventHandler(Client s, bool connected);

        /// <summary>
        /// 触发一个事件，通知订阅者客户端的状态已发生变化。
        /// </summary>
        /// <param name="connected">客户端的新连接状态。</param>
        ...
        /// 表示处理来自服务器消息的方法。
        /// </summary>
        /// <param name="s">接收到消息的客户端。</param>
        /// <param name="message">从服务器接收到的消息。</param>
        ...
        /// <param name="message">客户端已发送的消息。</param>
        /// <param name="messageLength">消息的长度。</param>
        ...
        /// 保活时间（毫秒）。
        /// </summary>
        public uint KEEP_ALIVE_TIME { get { return 25000; } } // 25秒

        /// <summary>
        /// 保活间隔（毫秒）。
        /// </summary>
        public uint KEEP_ALIVE_INTERVAL { get { return 25000; } } // 25秒

        /// <summary>
        /// 头部大小（字节）。
        /// </summary>
        public int HEADER_SIZE { get { return 4; } } // 4字节
        ...
        // 接收信息
        private int _readOffset;      // 读取偏移量
        private int _writeOffset;     // 写入偏移量
        private int _readableDataLen; // 可读数据长度
        private int _payloadLen;      // 负载长度
        private ReceiveType _receiveState = ReceiveType.Header; // 接收状态

        /// <summary>
        /// 互斥锁，防止在 <see cref="_stream"/> 上同时进行多个写入操作。
        /// </summary>
        ...
        private bool ValidateServerCertificate(object sender, X509Certificate certificate, X509Chain chain, SslPolicyErrors sslPolicyErrors)
        {
            ...
            // 比较接收到的服务器证书与包含的服务器证书，以验证我们连接到了正确的服务器
                        ...
                        case ReceiveType.Header: // 接收头部
                            {
                                ...
                                if (_readableDataLen + _writeOffset >= HEADER_SIZE)
                                {
                                    ...
                                    _receiveState = ReceiveType.Payload; // 切换到接收负载状态
                                }
                        ...
                        case ReceiveType.Payload: // 接收负载
                            {
                                ...
                                if (_writeOffset - HEADER_SIZE == _payloadLen)
                                {
                                    ...
                                    _receiveState = ReceiveType.Header; // 切换回接收头部状态
                                    _payloadLen = 0;
                                    _writeOffset = 0;
                                }
        ...
        /// <summary>
        /// 安全地发送消息，防止在 <see cref="_stream"/> 上同时进行多个写入操作。
        /// </summary>
        /// <param name="message">要发送的消息。</param>
        ...
        public void Disconnect()
        {
            if (_stream != null)
            {
                ...
                //_singleWriteMutex.Dispose(); // TODO: 通过创建新客户端来修复套接字重用问题
