...
namespace Quasar.Server.Networking
{
    public class QuasarServer : Server
    {
        ...
        /// 全局唯一实例
        /// </summary>
        private static QuasarServer _instance;

        /// <summary>
        /// 获取全局唯一实例
        /// </summary>
        public static QuasarServer Instance
        {
            get { return _instance; }
        }

        /// <summary>
        /// 初始化全局实例（在 FrmMain 中调用一次）
        /// </summary>
        public static void Initialize(X509Certificate2 serverCertificate)
        {
            if (_instance == null)
            {
                _instance = new QuasarServer(serverCertificate);
            }
        }
